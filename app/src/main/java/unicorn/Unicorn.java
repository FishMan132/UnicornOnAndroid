...
public class Unicorn implements UnicornConst, ArmConst, Arm64Const, M68kConst, SparcConst, MipsConst, X86Const {
    ...
    /**
     * Invoke all UC_HOOK_BLOCK callbacks registered for a specific Unicorn. This function gets invoked from the native
     * C callback registered for for UC_HOOK_BLOCK
     *
    ...
    /**
     * Invoke all UC_HOOK_INTR callbacks registered for a specific Unicorn. This function gets invoked from the native C
     * callback registered for for UC_HOOK_INTR
     *
     * @param eng A Unicorn uc_engine* eng returned by uc_open
    ...
    /**
     * Invoke all UC_HOOK_CODE callbacks registered for a specific Unicorn. This function gets invoked from the native C
     * callback registered for for UC_HOOK_CODE
     *
     * @param eng A Unicorn uc_engine* eng returned by uc_open
     * @param address The address of the instruction being executed
    ...
    /**
     * Invoke all UC_HOOK_MEM_XXX_UNMAPPED and/or UC_HOOK_MEM_XXX_PROT callbacks registered for a specific Unicorn. This
     * function gets invoked from the native C callback registered for for UC_HOOK_MEM_XXX_UNMAPPED or
     * UC_HOOK_MEM_XXX_PROT
     *
     * @param eng A Unicorn uc_engine* eng returned by uc_open
    ...
    /**
     * Invoke all UC_HOOK_MEM_READ callbacks registered for a specific Unicorn. This function gets invoked from the
     * native C callback registered for for UC_HOOK_MEM_READ
     *
     * @param eng A Unicorn uc_engine* eng returned by uc_open
     * @param address Address of instruction being executed
    ...
    /**
     * Invoke all UC_HOOK_MEM_WRITE callbacks registered for a specific Unicorn. This function gets invoked from the
     * native C callback registered for for UC_HOOK_MEM_WRITE
     *
     * @param eng A Unicorn uc_engine* eng returned by uc_open
     * @param address Address of instruction being executed
     * @param size Size of data being read
    ...
    /**
     * Invoke all UC_HOOK_INSN callbacks registered for a specific Unicorn. This is specifically for the x86 IN
     * instruction. This function gets invoked from the native C callback registered for for UC_HOOK_INSN
     *
     * @param eng A Unicorn uc_engine* eng returned by uc_open
    ...
    /**
     * Invoke all UC_HOOK_INSN callbacks registered for a specific Unicorn. This is specifically for the x86 OUT
     * instruction. This function gets invoked from the native C callback registered for for UC_HOOK_INSN
     *
     * @param eng A Unicorn uc_engine* eng returned by uc_open
     * @param port I/O Port number
    ...
    /**
     * Invoke all UC_HOOK_INSN callbacks registered for a specific Unicorn. This is specifically for the x86 SYSCALL and
     * SYSENTER instruction. This function gets invoked from the native C callback registered for for UC_HOOK_INSN
     *
     * @param eng A Unicorn uc_engine* eng returned by uc_open
    ...
    /**
     * Report the last error number when some API function fail. Like glibc's errno, uc_errno might not retain its old
     * value once accessed.
     *
     ...
     *
     * @param regid Register ID that is to be modified.
     * @param value Object containing the new register value. Long, BigInteger, or other custom class used to represent
     * register values
     */
     ...
     *
     * @param regid Register ID that is to be retrieved.
     * @return Object containing the requested register value. Long, BigInteger, or other custom class used to represent
     * register values
     */
     ...
     * @param timeout Duration to emulate the code (in microseconds). When this value is 0, we will emulate the code in
     * infinite time, until the code is finished.
     * @param count The number of instructions to be emulated. When this value is 0, we will emulate all the code
    ...
    /**
     * Stop emulation (which was started by emu_start() ). This is typically called from callback functions registered
     * via tracing APIs. NOTE: for now, this will stop the execution only after the current block.
     */
    ...
    /**
     * Hook registration for UC_HOOK_BLOCK hooks. The registered callback function will be invoked when a basic block is
     * entered and the address of the basic block (BB) falls in the range begin <= BB <= end. For the special case in
     * which begin > end, the callback will be invoked whenver any basic block is entered.
     *
    ...
    /**
     * Hook registration for UC_HOOK_INTR hooks. The registered callback function will be invoked whenever an interrupt
    ...
    /**
     * Hook registration for UC_HOOK_CODE hooks. The registered callback function will be invoked when an instruction is
     * executed from the address range begin <= PC <= end. For the special case in which begin > end, the callback will
     * be invoked for ALL instructions.
     *
    ...
    /**
     * Hook registration for UC_HOOK_MEM_READ hooks. The registered callback function will be invoked whenever a memory
     * read is performed within the address range begin <= read_addr <= end. For the special case in which begin > end,
     * the callback will be invoked for ALL memory reads.
     *
    ...
    /**
     * Hook registration for UC_HOOK_MEM_WRITE hooks. The registered callback function will be invoked whenever a memory
     * write is performed within the address range begin <= write_addr <= end. For the special case in which begin >
     * end, the callback will be invoked for ALL memory writes.
     *
     ...
     * invoked whenever a memory write or read is performed within the address range begin <= addr <= end. For the
     * special case in which begin > end, the callback will be invoked for ALL memory writes.
     *
    ...
    /**
     * Hook registration for UC_HOOK_MEM_XXX_UNMAPPED and UC_HOOK_MEM_XXX_PROT hooks. The registered callback function
     * will be invoked whenever a read or write is attempted from an invalid or protected memory address.
     *
    ...
    /**
     * Hook registration for UC_HOOK_INSN hooks (x86 IN instruction only). The registered callback function will be
     * invoked whenever an x86 IN instruction is executed.
     *
    ...
    /**
     * Hook registration for UC_HOOK_INSN hooks (x86 OUT instruction only). The registered callback function will be
     * invoked whenever an x86 OUT instruction is executed.
     *
    ...
    /**
     * Map existing host memory in for emulation. This API adds a memory region that can be used by emulation.
     *
     * @param address Base address of the memory range
     * @param size Size of the memory block.
     * @param perms Permissions on the memory block. A combination of UC_PROT_READ, UC_PROT_WRITE, UC_PROT_EXEC
     * @param ptr Block of host memory backing the newly mapped memory. This block is expected to be an equal or larger
     * size than provided, and be mapped with at least PROT_READ | PROT_WRITE. If it is not, the resulting behavior is
     * undefined.
     */
    ...
    /**
     * Retrieve all memory regions mapped by mem_map() and mem_map_ptr() NOTE: memory regions may be split by
     * mem_unmap()
     *
    ...
    /**
     * Allocate a region that can be used with uc_context_{save,restore} to perform quick save/rollback of the CPU
     * context, which includes registers and some internal metadata. Contexts may not be shared across engine instances
     * with differing arches or modes.
     *
    ...
    /**
     * Free a resource allocated within Unicorn. Use for handles allocated by context_alloc.
     *
    ...
    /**
     * Save a copy of the internal CPU context. This API should be used to efficiently make or update a saved copy of
    ...
    /**
     * Restore the current CPU context from a saved copy. This API should be used to roll the CPU context back to a
     * previous state saved by uc_context_save().
     *
     * @param context handle previously returned by context_alloc.
     */
